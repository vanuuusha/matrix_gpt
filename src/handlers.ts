import ChatGPTClient from '@waylaidwanderer/chatgpt-api';MessageEvent
...
export default class CommandHandler {
  ...
  constructor(private client: MatrixClient, private chatGPT1: ChatGPTClient, private chatGPT2: ChatGPTClient) { }
  ...
  private async onMessage(roomId: string, event: MessageEvent) {
    try {
      ...
      if (storedConversation != undefined) {
          model = storedConversation.config.model;
      }
      
      if (event.content.body.trim().startsWith('gpt4') || model == 'gpt4') {
        model = 'gpt4'
        var text = await bodyWithoutPrefix;
        text = text.slice(4,);
        result = await sendChatGPTMessage(this.chatGPT2, text, storedConversation)
        .catch((error) => {
        ...
        model = 'gpt3'
        result = await sendChatGPTMessage(this.chatGPT1, await bodyWithoutPrefix, storedConversation)
        .catch((error) => {
          LogService.error(`OpenAI-API Error: ${error}`);
          sendError(this.client, `The bot has encountered an error, please contact your administrator (Error code ${error.status || "Unknown"}).`, roomId, event.event_id);
        });
      ...
      const storedConfig = ((storedConversation !== undefined && storedConversation.config !== undefined) ? storedConversation.config : {model: model})
